...
new_matrix_with_entry :: proc(rows: int, cols: int, entries: []f64) -> Matrix {
	assert(len(entries) == rows * cols, "Length of matrix entries should be equal to cols * rows")
...
mult_matrix_by_tuple :: proc(m: Matrix, t: Tuple) -> Tuple {
	a := read_matrix(m, 0, 0) * t.x + read_matrix(m, 0, 1) * t.y + read_matrix(m, 0, 2) * t.z + read_matrix(m, 0, 3) * t.w
	b := read_matrix(m, 1, 0) * t.x + read_matrix(m, 1, 1) * t.y + read_matrix(m, 1, 2) * t.z + read_matrix(m, 1, 3) * t.w
	c := read_matrix(m, 2, 0) * t.x + read_matrix(m, 2, 1) * t.y + read_matrix(m, 2, 2) * t.z + read_matrix(m, 2, 3) * t.w
	d := read_matrix(m, 3, 0) * t.x + read_matrix(m, 3, 1) * t.y + read_matrix(m, 3, 2) * t.z + read_matrix(m, 3, 3) * t.w
